...
def check_position_range(sequence: list[int], n_vehicles: int, verbose: bool = False) -> bool:
    ...
    if min(sequence) < 0 or max(sequence) > n_vehicles - 1:  # 0-based indexing
        if verbose:
            print(f"Warning: The sequence is not valid: {sequence} (elements must be between 0 and {n_vehicles-1})")
        return False
    return True
...
def check_sequence_equality(entry: list[int], exit: list[int], verbose: bool = False) -> bool:
    ...
    if entry != exit:
        if verbose:
            print(f"Warning: The entry and exit sequences are not valid: {entry} != {exit} (sequences must be the same)")
        return False
    return True
